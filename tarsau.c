...
int selectTransaction(int argc, char *argv[]){
    ...
    else if (strcmp(argv[1], "-a") == 0)
    {
        ...
        if (strstr(argv[2], ".sau") != NULL)
        {
            ...
            extractArchiveFile(archiveFileName, outputFolderName);
        }
        else
        {
...
int isTextFile(const char *fileName) {
    ...
    return 0; //The file could not open or read
}
...
void extractArchiveFile(char *archiveFileName, char *outputFolderName) {
    ...
    printf(" => This directory has %d merged files\n \n", fileCount);
